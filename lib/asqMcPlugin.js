...
module.exports = ASQPlugin.extend({
  ...
  parseOptions: function($, el){
    ...
    // if using asq-option and asq-option-other separately
    // var $asqOthers = $el.find('asq-option-other');
    // $asqOptions = $asqOptions.concat($asqOthers);

    assert($asqOptions.length > 1
      , 'A multi-choice question should have at least two `asq-option` or `asq-option-others` children' )
